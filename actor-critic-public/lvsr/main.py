...
def create_model(config, data, load_path=None, test_tag=False):
    ...
    if load_path:
        ...
        for brick_path, attribute_dict in sorted(
                ...
                key=lambda k: k[0].count('/')):
                #key=lambda (k, v): k.count('/')): # modified by Fence
...
def initialize_all(config, save_path, bokeh_name, params, bokeh_server, bokeh, test_tag, use_load_ext, load_log, fast_start):
...
def train(config, save_path, bokeh_name, params, bokeh_server, bokeh, test_tag, use_load_ext, load_log, fast_start, debug_mode):
...
def train_multistage(config, save_path, bokeh_name, params, start_stage, final_stage, **kwargs):
...
def search(config, params, load_path, part, decode_only, report, decoded_save, nll_only, seed):

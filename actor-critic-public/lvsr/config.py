...
class Configuration(dict):
    ...
    def __init__(self, config_path, schema_path, config_changes):
        ...
        if self.multi_stage:
            ...
            ordered_changes = OrderedDict(
                sorted(stages, key=lambda v: v[1]['number'],))
                #sorted(stages, key=lambda (k,v): v['number'],))# modified by Fence
    ...
    def stage_number(self, stage_name):
        """Return the serial number of the stage."""
        return self.ordered_stages.keys().index(stage_name)
